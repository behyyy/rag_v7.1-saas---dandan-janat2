...
def get_avalai_completion(prompt, avalai_api_key, model_name="gpt-4.1-nano", max_tokens=100):
    ...
    try:
        response = client.chat.completions.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "شما دستیار هوشمند کلینیک دندانپزشکی جنت هستید. لطفاً با توجه به متن مرجع زیر، به سوال پاسخ بده. اگر پاسخ دقیق وجود ندارد، نزدیک‌ترین اطلاعات مرتبط را ارائه کن. نیازی به معرفی مجدد کلینیک نیست، فقط روی پاسخگویی تمرکز کن."},
        ...
        return "متاسفانه در حال حاضر قادر به پاسخگویی نیستم. لطفاً بعداً امتحان کنید یا با کلینیک جنت تماس بگیرید."
...
if __name__ == "__main__":
    ...
    force_rebuild = False
    user_input = input("آیا می‌خواهید کش را بازسازی کنید و وضعیت خواندن PDFها را ببینید؟ (y/n): ").strip().lower()
    if user_input == 'y':
        force_rebuild = True
    current_state = get_pdf_state(pdf_folder)
    cached_state = load_pdf_state()
    need_rebuild = (current_state != cached_state) or force_rebuild
    if not need_rebuild:
    ...
    if need_rebuild:
        ...
        for pdf_file in pdf_files:
            ...
            if not extracted_text.strip():
                print(f"⚠️ هشدار: هیچ متنی از '{pdf_file}' استخراج نشد!")
            else:
                print(f"✅ تعداد کاراکتر استخراج‌شده از '{pdf_file}': {len(extracted_text)}")
